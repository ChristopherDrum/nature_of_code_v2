...
typedef struct WalkerOps WalkerOps;
...
struct WalkerOps {
	void (*StepWalker)(Walker *);
	void (*DrawWalker)(Walker *);
};
WalkerOps wops = {0};
...
int main(void) 
{
	...
	wops.DrawWalker = draw;
	wops.StepWalker = step;
	...
	crowd[0] = (Walker){{25,25},{25,25},{0.0,2000.0},ORANGE, &wops};
	crowd[1] = (Walker){{100,75},{100,75},{1000.0,6000.0},GREEN, &wops};
	crowd[2] = (Walker){{175,125},{175,125},{5000.0,10000.0},YELLOW, &wops};
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
		...
		BeginTextureMode(virtualScreen);
			...
			{
				Walker *walker = &crowd[i];
				walker->ops->StepWalker(walker);
				walker->ops->DrawWalker(walker);
			}
		EndTextureMode();
